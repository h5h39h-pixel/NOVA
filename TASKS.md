...
| HON‑1b | Per‑action confirmation mode for autonomous control (optional) | P1 | ⬜ | Kill‑switch (HON‑1) + dry‑run exist; a "confirm each control action" mode would add finer supervision. |
...
| HON‑2c | **Switch control typing from pyautogui → keysender/SendInput + clipboard** | P1 | ⬜ | Root cause of the GUI‑control unreliability: `control.type_text`/`act_on_screen` use pyautogui, which mangles/drops keys on this machine (CLAUDE.md already noted this — the known‑good path is keysender hardware input + real Ctrl+V, as mcp‑control uses). Replacing the input backend is the real fix for reliable GUI control. |
