...
| HON‑9 | **CI actually running** | ✅ | M94. CI now **auto‑runs locally**: the gate fires on every **commit** (`.githooks/pre-commit`) AND every **push** (new `.githooks/pre-push`, verified); `scripts/ci_local.py` does the clean‑venv CI; `.github/workflows/ci.yml` validated (parses, 5 steps, runs‑on windows‑latest). The only remaining piece — GitHub‑*hosted* execution — needs a remote the owner creates; the workflow is ready for it. |
