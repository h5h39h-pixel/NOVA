...
|---|---|---|---|
| IDEA‑1 | Visual macro recorder (record/replay UIA+mouse as named macros) | P2 | 🟧 M105. **Replay half shipped:** a workflow `control` step drives mouse/keyboard via the control service (gated by `exec_allowed` + panic stop), so a workflow of control steps = a replayable macro (move/click/drag/scroll/type/keys/click_element/set_text). Test ✅. **Recording half deferred (honest):** passive global mouse/keyboard capture needs a new dep (pynput / Win32 hooks) + is privacy‑sensitive, and this env suppresses synthetic keyboard (UIA SetValue only) — so a live recorder + a builder‑UI form for control steps are not shipped. Build macros via `/api/workflows` (steps with `action:control`) for now. |
...
| IDEA‑4 | Hands‑free voice conversation loop (STT→answer→TTS) | P2 | ✅ M105 (🟧 live‑loop). Unified Workspace 🎙️ toggle: listens (MediaRecorder + Web‑Audio **silence detection**, 15s cap) → `/api/stt` → sends a chat turn → on reply‑end speaks it via local Piper `/api/tts` → listens again; stops on page‑leave/error/toggle. All three legs verified (STT M104, chat, TTS now `ok=True`); renders zero console errors. **Honest:** the continuous mic loop can't be driven headlessly, so it's component‑verified, not end‑to‑end automated. |
